<?php
	...
	if (isset($data['newtask']))  {
		...
		if (empty($errors)) {
			...
			//Преобразование данных, полученный из чекбокса формы, связанной с категориями
			if (isset($data['category'])) {
				foreach ($data['category'] as $value) {
					$categories = implode('|', $data['category']); //На выходе могут быть следующие значения: 0|0, 0|1, 1|0, 1|1
				};
			};
			...
			$task->created = date("m.d.y"); //Дата и время создания задачи. Заполняется автоматически и изменению не подлежит

			if ($data['startd']) {
				$task->startd = $data['startd']; //Дата начала задачи. Необязательное поле
			};

			if ($data['startt']) {
				$task->startt = $data['startt']; //Время начала задачи. Необязательное поле
			};

			if ($data['finishd']) {
				$task->finishd = $data['finishd']; //Дата завершения задачи. Необязательное поле
			};

			if ($data['finisht']) {
				$task->startt = $data['finisht']; //Время завершения задачи. Необязательное поле
			};
			...
			if ($categories) {
				$task->categories = $categories; //Категории
			};
			R::store($task);
		};
	};
	...
	print "<form action=\"\" method=\"POST\">
			...
			<p>Категории<br>";
	$cat = R::find('categories', 'login LIKE ?', array($login));
	$i = 0;
	...
	print "</p>
			...
			<p><input type=\"submit\" name=\"newtask\" value=\"Создать\">
		</form>";
	foot();
?>